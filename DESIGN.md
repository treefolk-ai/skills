...
Each run discovers candidates from current friction, then compares their meaning with prior advice and user-checked actions. The user asked for useful details beyond familiar entry points such as Plan; references support claims without defining a fixed recommendation pool. Random selection displays at most three eligible feature candidates, remains stable within a report, and imports only displayed advice into later history. Recurring topics require a concrete new detail; an honest empty result is valid. The separate App / CLI quick reference is not counted as a new finding.

The top disclosure preserves original advice, dates and source links across periods, respecting project scope and excluding uncertain attribution, demos and incomplete reports. Scripted exact deduplication and bounded semantic grouping must not recursively import copied history or imply adoption. Known actions come only from the shared checkbox file; feature-use detection and inferred familiarity are excluded. Token deltas share the session scan and date/project filters, with weighted cache rates and visible missing coverage. Accounting details and schema-v1 compatibility belong in the package’s data-format reference; counts are not efficiency scores.

Reports live at `${TREEFOLK_HOME:-~/.treefolk}/insights/<generation-time>-<unique-suffix>/`, independent of the checkout, activation path and `CODEX_HOME`. Each private folder contains data and HTML, is delivered with both links, and can be removed independently; earlier reports and permissions are preserved. Failed writes never silently redirect to project caches or temporary folders. There is no automatic retention, report index or uninstall-time data deletion.

The user chose direct browser file access to avoid a running service. All reports share `known.md` at the report root; collection creates a missing empty file only when data exists. The report defaults to reading mode instead of repeating a checkbox on every card. “整理建议” opens a batch selection mode; choices remain a page-local draft until the user saves, which is when file permission is requested. One exclusive write rereads the file and applies the batch, preserving other choices and notes. Cancel discards the draft, failure preserves it and the source file, and successful known items remain visible with an undo path. The file is authoritative; a cached handle only assists reconnection. Browser support and permissions vary, new reports may need reselection, and a copied file does not automatically reach the next collector. Removing one report leaves the shared record intact. The exact file protocol and error behavior belong in the package reference.

The white report uses fine dividers, single-column advice, blue action highlights and amber limitations. Evidence expands on demand while key limits stay visible. Statistics respond to filters; narrative retains its analysis period, with a copyable invocation for re-analysis. Maintenance visual acceptance uses rendered desktop and narrow-screen output; ordinary invocations reuse the template.

To reduce caller tokens, normal invocation ends after successful generation and returned paths/statuses, without probes, separate checks, tests, demos or browser acceptance. Built-in input validation, escaping, redaction and overwrite protection remain. Runtime helpers live in `scripts/`; maintenance tests and the synthetic report fixture live in `tests/`. The fixture requires an explicit test directory. Package maintenance uses `python3 -B skills/insights/tests/test_insights.py`, `node skills/insights/tests/test_feedback.cjs` and the repository checks in `AGENTS.md`. File API doubles do not verify browser permission dialogs, and maintenance checks do not establish advice effectiveness.
