...
The durable implementation policy is to justify added entities by a current benefit, use project and ecosystem capabilities first, and keep verification proportional. File length, multiple change reasons and side effects are signals for judgment, not automatic splitting rules. Existing toolchains take precedence over personal defaults; pnpm is the default only for JavaScript/TypeScript work with no established package manager and compatible constraints. Choosing a tool does not authorize installing it, accessing the network or migrating a project. Failure verification applies only to real affected failure semantics. The complete decisions and workflow live in `skills/build/SKILL.md`; broad behavior-preserving cleanup, requirements exploration, read-only review, Git delivery and deployment are separate outcomes.
