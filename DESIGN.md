...
`push` defaults to a streamlined path: it starts immediately, inspects state once, skips pre-push fetch/history comparison and detailed commit metadata checks, and verifies delivery with a lightweight `git ls-remote` presence check. This default still keeps every Safety and Stop condition, including the non-force push and the staged-diff secret check; it trades the extra belt-and-suspenders verification for speed on the common single-author fast-forward case. `$push safe` opts into the full verification path (plan, pre-push history comparison, commit style lookup, identity preflight, and post-push ahead/behind confirmation) for shared or complex repositories. This is a deliberate default favoring speed; it does not weaken the prohibition on force pushes or unreviewed staging.
