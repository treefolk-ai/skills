...
`insights` owns the recurring question “what could improve how I work with AI?” The user identified personal usage review, repeated friction, useful feature discovery and an attractive HTML report as one job, inspired by Claude Code's `/insights`. The short name makes this request recognizable. `todo` selects documented project work; `human-in-the-loop` records one task's evidence; `grill` examines a proposed decision. None owns retrospective analysis of personal usage across sessions, so this is a separate `think / ai-workflow / synthesis` entry. Statistics, source references and a rendered report make the result inspectable; interpretation and matching documented capabilities to observed needs provide value beyond a command wrapper. Repeated use must still establish the quality of its advice.

The user requested selectable periods, including 90 days for an initial review, and a switch for reading current features. Default to 90 days without creating first-run tracking state, and reuse an explicit period from the current conversation. The latest-features switch defaults on and authorizes only generic official documentation queries, never uploading transcripts. Turning it off uses existing materials without probing local capabilities. Query failure is a visible partial outcome, not a silent offline success. Local helpers never access the network; model analysis belongs to the active host, so the whole workflow is not advertised as offline.

`insights` is P0 because personal history review crosses project boundaries and must be deliberately selected. Invocation covers the documented history scope, private local outputs and the enabled official-query path. It does not authorize applying recommendations, modifying global guidance, installing, publishing or Git delivery. The complete workflow lives in its `SKILL.md`; a Python standard-library collector, renderer and HTML asset support deterministic steps. The first data adapter supports Codex rollouts only. The Chinese static report filters collected statistics by date and project; narrative retains its original analysis period. A new period or feature-switch choice produces a copyable invocation for actual re-analysis. No data, no warranted change, incomplete coverage and failed prerequisites are separately reported; no report or preference is overwritten automatically.

Personal reports belong to a stable user-owned location: `${TREEFOLK_HOME:-~/.treefolk}/insights/<generation-time>-<unique-suffix>/`. The user rejected a report hidden in the skill checkout's `.cache`, because it was difficult to find and belonged to the development environment. The collector now creates the default directory deterministically; the renderer keeps HTML beside the collected data, and delivered examples follow the same convention. Working directory, skill activation path and `CODEX_HOME` do not choose the destination. Every completion provides both report and folder links; removing that folder removes one report without touching source sessions or installed skills. New outputs do not overwrite earlier ones, and write failures never silently redirect output into a repository or temporary directory. This adds no report index, automatic retention or uninstall-time deletion; isolated maintenance tests may explicitly use temporary output roots.

The report favors a minimal white surface, fine dividers and single-column recommendations. The user rejected the decorative green presentation and requested a fresh UI with important text highlighted. Blue highlights identify concrete next actions; amber identifies unverified capabilities and incomplete coverage. Supporting reasoning and evidence expand on demand, while consequential limitations remain visible. Visual acceptance when maintaining the template uses the rendered desktop and narrow-screen report; ordinary invocations reuse that template.

The user requested lower token cost for each `$insights` invocation. Its completion boundary is successful report generation with the helpers' returned paths and statuses. Runtime omits environment and capability probes, separate artifact checks, tests, demos and browser acceptance; absence of these checks alone is not a partial outcome. Built-in validation, escaping, redaction and overwrite protection remain part of generation. Package maintenance stays separate: helper or template changes use `python3 -B skills/insights/scripts/test_insights.py`, and repository changes follow the complete checks in `AGENTS.md`. Maintenance checks do not prove that a personal report's advice is effective.
