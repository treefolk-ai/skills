...
- **P0：已启用，仅显式调用。** 用户必须写出 `$skill-name`。适合会提交代码、推送远端、创建仓库、发起 Pull Request 或部署到外部运行环境的高影响工作流，以及 `loop` 这样与多种产物工作流重叠、会持续派发任务的入口。
...
> **P0 用户须知：** 安装或启用 P0 skill，不代表它会参与基于描述的自动语义匹配。普通自然语言请求不会让模型自动选中它；用户必须输入准确命令。P0 skill 仍可出现在宿主的 skill 列表或命令补全中。当前 Codex adapter 使用 `$repo`、`$push`、`$pr`、`$deploy` 和 `$loop`。当前 `setup` 尚不支持 Claude Code，也不安装 Ralph Wiggum 或 `loop` 的宿主 Stop hook。
