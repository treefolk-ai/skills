...
忘了做到哪里、准备暂停，或想保留验收反馈时，调用 `$human-in-the-loop`。人和 AI 共用项目根目录的 `evidence.md`：目标、上次验证结果、已知问题、下一次验证，四个字段、每项通常一至两句。你可以只看，也可以随手记一句，由 AI 核对整理；愿望不会被写成已验证的成果。没有变化就不重写，记录后继续已授权的工作；`$todo` 仍负责选择下一件事。下一次可说“读取 evidence.md，帮我接上这个项目”；明确要求项目接入时，可在 `AGENTS.md` 留一句入口指引，不保证宿主自动读取。完整规则见[技能说明](skills/human-in-the-loop/SKILL.md)。
