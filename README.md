...
`$insights` 默认回顾本机 Codex 最近 90 天的会话，可指定天数、起止日期或项目；`最新功能开/关` 控制生成时是否查询官方最新功能。报告包含日期/项目统计筛选、会话依据和可复制的下一次调用。当前采集器支持 Codex JSONL，需要 Python 3.9+；网页为中文本地静态文件，AI 分析由当前宿主完成，其他宿主的数据适配尚未实现。

日常调用只做采集、分析、按开关查询资料和网页生成，生成后直接交付；不做本机能力检查、测试或浏览器验收，减少调用者的 token 消耗。

报告采用清爽白底，突出行动建议与关键限制，依据按需展开。统一存放在 `~/.treefolk/insights/<生成时间>-<唯一后缀>/report.html`（设置 `TREEFOLK_HOME` 时随之调整）。每次交付附网页和文件夹入口；整夹移到废纸篓即可清理，技能源码与原始会话不受影响。
